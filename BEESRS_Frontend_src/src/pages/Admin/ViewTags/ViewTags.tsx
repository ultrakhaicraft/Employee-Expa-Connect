...
import { Search, ChevronLeft, ChevronRight, Edit, Trash2, Hash, Plus, RefreshCw } from 'lucide-react'
import { GetTags, CreateTag, EditTag, DeleteTag } from '@/services/adminService'
...
export default function ViewTags() {
  ...
  const [selectedTagForDelete, setSelectedTagForDelete] = useState<Tag | null>(null)
  ...
  const handleConfirmEdit = async () => {
    ...
    try {
      ...
      toast({
        title: 'Update failed',
        description: e?.response?.data?.message || 'Unable to update tag. Please try again.',
        variant: 'destructive' as any,
      })
    }
  }
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: e?.response?.data?.message || 'Unable to create tag. Please try again.',
        variant: 'destructive' as any,
      })
    }
  }
  ...
  const handleConfirmDelete = async () => {
    if (!selectedTagForDelete) return
    try {
      await DeleteTag(selectedTagForDelete.tagId)
      toast({
        title: 'Delete successful',
        description: `${selectedTagForDelete.name} has been deleted successfully.`,
      })
      ...
      toast({
        title: 'Delete failed',
        description: e?.response?.data?.message || 'Unable to delete tag. Please try again.',
        variant: 'destructive' as any,
      })
    }
  }
      ...
      <PageTransition>
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-3xl font-bold text-[#1F2937]">Tag Management</h1>
          </div>
          ...
          <Card className="border border-[#E5E7EB] shadow-sm bg-white">
            <CardContent className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <Button onClick={fetchTags} className="bg-[#D9822B] hover:bg-[#c17424] text-white">
                  Try Again
                </Button>
              </div>
            </CardContent>
    ...
    <PageTransition>
      ...
      <AlertDialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <AlertDialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[#0B1F3A] mb-2">Tag Name</label>
              <Input
                value={editForm.name}
      ...
      <AlertDialog open={confirmEditDialogOpen} onOpenChange={setConfirmEditDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='mb-4 text-base'>
              Are you sure you want to update "{editForm.name}"? This will modify the tag information.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className='mt-5'>
      ...
      <AlertDialog open={confirmCreateDialogOpen} onOpenChange={setConfirmCreateDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='mb-4 text-base'>
              Are you sure you want to create "{addForm.name}"? This will add a new tag to the system.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className='mt-5'>
            <AlertDialogCancel onClick={() => {
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className='mb-4 text-base'>
              Are you sure you want to delete "{selectedTagForDelete?.name}"? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className='mt-5'>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-[#1F2937]">Tag Management</h1>
            <p className="text-[#7A869A] mt-2">Manage and view all place tags in the system</p>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedTags.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-[#9AA3B5]">
                        No tags found
                      </TableCell>
                    </TableRow>
                  ) : (
                    paginatedTags.map((tag) => (
                      <TableRow key={tag.tagId} className="hover:bg-[#F9F9F9]">
                        <TableCell className="text-[#1F2937] font-medium">{tag.tagId}</TableCell>
                        <TableCell className="text-[#1F2937] font-medium">{tag.name}</TableCell>
                        <TableCell className="text-[#7A869A]">{tag.description || '-'}</TableCell>
                        <TableCell>
                          <Badge 
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-6">
                <div className="text-sm text-[#7A869A]">
                  Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, filteredTags.length)} of {filteredTags.length} results
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
